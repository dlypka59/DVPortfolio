...
void CVideoView::RefreshStatusBarForThisView()
{
	if (m_viewStatus.GetSafeHwnd())
	{
		...
		else
		{
			...
			if (total > 0)
			{
				text.Format(_T("Frame %lld / %lld   %s   file %.3f fps   play %.0f fps"),
					frame, total, timeText.GetString(), fps, playFps);
			}
			else
			{
...
void CVideoView::ApplySeekSliderToFrame()
{
	...
	SetCurrentFrame(static_cast<int64_t>(m_seekSlider.GetPos()));
}

void CVideoView::SyncFpsSliderFromMain()
{
	...
	double fps = 30.0;
	if (CMainFrame* pMain = dynamic_cast<CMainFrame*>(AfxGetMainWnd()))
		fps = pMain->GetPlayFps();

	int pos = static_cast<int>(fps + 0.5);
	if (pos < 1) pos = 1;
	if (pos > 240) pos = 240;

	m_updatingFpsSlider = true;
	if (m_fpsSlider.GetPos() != pos)
		m_fpsSlider.SetPos(pos);
	m_updatingFpsSlider = false;
}
...
int CVideoView::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	...
	m_fpsSlider.ShowWindow(SW_SHOW);
...
void CVideoView::LayoutBottomControls(int cx, int cy)
{
	...
	const int bottomH = kFpsHeight + kSeekHeight + kViewStatusHeight;
	int y = cy - bottomH;
	if (y < 0)
		y = 0;
	...
	if (m_viewStatus.GetSafeHwnd())
		m_viewStatus.MoveWindow(0, y + kFpsHeight + kSeekHeight, cx, kViewStatusHeight);
}
...
void CVideoView::OnInitialUpdate()
{
	...
	SetFocus();
	RefreshStatusBarForThisView();
	SyncFpsSliderFromMain();
	Invalidate(FALSE);

	if (CMainFrame* pMain = dynamic_cast<CMainFrame*>(AfxGetMainWnd()))
	{
		if (HasVideo())
			pMain->InitPlayFpsFromVideo(GetFrameRate());
		pMain->UpdateViewCountStatus();
	}
}
...
void CVideoView::OnHScroll(UINT nSBCode, UINT nPos, CScrollBar* pScrollBar)
{
		...
		pScrollBar->GetSafeHwnd() == m_seekSlider.GetSafeHwnd())
	{
		switch (nSBCode)
		{
		case TB_LINEUP: case TB_LINEDOWN:
		case TB_PAGEUP: case TB_PAGEDOWN:
		case TB_THUMBTRACK: case TB_THUMBPOSITION:
		case TB_TOP: case TB_BOTTOM: case TB_ENDTRACK:
	...
	if (pScrollBar != nullptr &&
		pScrollBar->GetSafeHwnd() == m_fpsSlider.GetSafeHwnd())
	{
		if (m_updatingFpsSlider)
			return;
...
void CVideoView::StartPlayback()
{
	...
	UINT interval = static_cast<UINT>(1000.0 / playFps + 0.5);
	if (interval < 1)
		interval = 1;
...
CVideoDoc* CVideoView::GetDocument() const
{
